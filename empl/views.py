...
def add_employees(request):
    ...
    if form.is_valid():
        ...
        messages.add_message(request, messages.INFO, f"{form.cleaned_data.get('name')} a été ajouté")
...
def delete_employee(request, id=None):
    ...
    if request.method == "POST":
        emp.delete()
        messages.add_message(request, messages.INFO, f"Employé {emp.name} Supprimé")
        return redirect('allEmp')
    context = {
        'emp': emp
    }
